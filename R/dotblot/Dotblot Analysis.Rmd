...
```{r}
dotblot_train <- read.csv("../train_dotblot.csv")
dotblot_test <- read.csv("../test_dotblot.csv")
...
merged_train <- merge(dotblot_train, outcome_train, by = "ID")
merged_test <- merge(dotblot_test, outcome_test, by = "ID")
...
```{r}
train_msa <- reduceByCorrelationWithinGroups(train_msa, threshold = 0.5)
...
# MODEL SO BAD WE CANNOT GET A AUC?????
#testbestAUC <- getElasticNetPerformance(strict_Predicted_response, as.numeric(test_msa$outcome))
